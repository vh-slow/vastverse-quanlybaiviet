...
export default function PublicProfilePage() {
    ...
    const handleToggleFollow = async () => {
        ...
        try {
            if (isFollowing) {
                await apiUser.unfollowUser(user.id);
                ...
                await apiUser.followUser(user.id);
                setIsFollowing(true);
                setUser((prev: any) => ({
                    ...prev,
                    followersCount: prev.followersCount + 1,
    ...
    const openFollowModal = async (type: 'followers' | 'following') => {
        ...
        try {
            const data =
                type === 'followers'
                    ? await apiUser.getFollowers(user.username)
                    : await apiUser.getFollowing(user.username);
            setFollowData(data);
        } catch (error) {
        ...
        <>
                        ...
                        <div className="flex flex-col px-4 sm:px-8 relative -mt-12 sm:-mt-16 mb-8">
                            <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between gap-4 sm:gap-6">
                                ...
                                <div className="pb-2 text-center sm:text-left flex-1">
                                    <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 leading-tight flex items-center justify-center sm:justify-start gap-2">
                                        {user.fullName}
                                        {user.role === 'admin' && (
                                            <i
                                                className="fa-solid fa-circle-check text-blue-500 text-xl"
                                ...
                                {currentUser?.username !== user.username && (
                                    <div className="pb-2 sm:shrink-0">
                                        <button
                                            onClick={handleToggleFollow}
                                            disabled={isFollowLoading}
                                            className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2 min-w-[140px] shadow-sm group ${
                                                isFollowing
                                            ...
                                            {isFollowLoading ? (
                                                ...
                                                <>
                                                    <span className="group-hover:hidden">
                                                        <i className="fa-solid fa-user hidden group-hover:inline-block mr-2"></i>
                                                        Đang theo dõi
                                                    </span>
                                                    <span className="hidden group-hover:inline-block">
                                                        <i className="fa-solid fa-user-minus hidden group-hover:inline-block mr-2"></i>
                                                        Bỏ theo dõi
                                                    </span>
                                                </>
                                            ) : (
            ...
            {isFollowModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl flex flex-col max-h-[75vh] relative z-10 animate-zoom-in">
